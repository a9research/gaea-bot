...
现在需要新增一个获取每日奖励的功能，每天在训练后执行一次领取，首先访问接口：https://api.aigaea.net/api/reward/daily-list，请求方式和其他几个接口一样，没有request payload。
返回格式如下：
{
    "code": 200,
    "success": true,
...
访问接口：
https://api.aigaea.net/api/reward/daily-complete 
request payload 格式为：{"id":7}，这里的id的值就是刚刚daily-list接口返回的 "daily"的值，随机选取一个reward为空的对应的"daily"的值作为id去请求daily-complete 接口。返回值如下：
{
    "code": 200,
    "success": true,
    "msg": "Success",
